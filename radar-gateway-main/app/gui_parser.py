...
class UARTParser():
    ...
    # This function is called to create logs files from command line. 
    # It prints the HEX data and the frame parsed with time stamp
    def logParsing(self):
        ...
        while (1):
            ...
            # This check ensures we can give a meaningful error
            if (len(magicByte) < 1):
                log.error("ERROR: Read timed out")
                magicByte = self.dataCom.read(1)
        ...
        # Return parsed packet and raw packet
    ...
    def connectComPorts(self, cliCom, dataCom):
        if(self.device == "xWR6843"):
            ...
            self.dataCom = serial.Serial(dataCom, 921600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=0.6, inter_byte_timeout=0.1)
    ...
    # Connect to Data COM port
    def connectDataComPort(self, dataCom):
        if(self.device == "xWR6843"):
            self.dataCom = serial.Serial(dataCom, 921600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=10)
            self.dataCom.reset_output_buffer()
            log.info('Connected')
        elif(self.device == "xWRL6844"):
            self.dataCom = serial.Serial(dataCom, 1250000, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=10)
            self.dataCom.reset_output_buffer()
            log.info('Connected')
            
            
        
    # Send single byte to device over UART COM.
    def sendByte(self, b):
            self.dataCom.write(b)
